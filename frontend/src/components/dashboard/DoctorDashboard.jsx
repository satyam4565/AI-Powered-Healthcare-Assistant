...
import { 
  ...
  CreditCard, User, Settings, LogOut, ChevronLeft, ChevronRight, 
...
export const DoctorDashboard = ({ data }) => {
  ...
  const [bio, setBio] = useState(user?.bio || 'General Practitioner with 10+ years of experience. Specializing in preventative medicine and routine checkups.');
  ...
  const reviewData = data?.reviews || { excellent: 0, great: 0, good: 0, average: 0 };
  ...
  const handleSaveBio = async () => {
    ...
    try {
      ...
      updateUser({ bio, specialization, experience, location, profileImage });
  ...
  return (
    <div className={isDarkMode ? 'dark' : ''}>
      <div className="flex flex-col md:flex-row h-full min-h-screen bg-[#F5F6FA] dark:bg-slate-950 transition-colors duration-300">
        ...
        <aside className="w-full md:w-64 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 flex flex-col shrink-0">
          <div className="p-8 flex flex-col items-center">
            ...
            <h3 className="font-bold text-blue-600 dark:text-blue-400 text-lg">{user?.name || 'Doctor'}</h3>
            <p className="text-[10px] text-blue-500/70 font-semibold uppercase tracking-wider text-center mt-1">
          ...
          <div className="flex-1 overflow-y-auto">
            ...
            <div className="px-4 py-2 mt-2">
              <button onClick={logout} className="flex items-center gap-4 w-full px-8 py-3 text-slate-600 dark:text-slate-400 hover:text-red-500 transition-colors font-medium text-sm rounded-r-full hover:bg-slate-50 dark:hover:bg-slate-800/50">
          ...
          <AnimatePresence mode="wait">
                ...
                <div className="xl:col-span-2 space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <StatCard
                      ...
                      value={todayAppointmentsCount.toString().padStart(3, '0')}
                      subtext={today.toLocaleDateString(undefined, { day: 'numeric', month: 'short', year: 'numeric' })}
                      icon={<Activity />}
                    />
                  </div>
                    ...
                    <Card className="p-6 border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm">
                      <div className="flex justify-between items-center mb-6">
                        ...
                        <div className="flex items-center gap-2 text-sm font-semibold text-blue-600">
                          <button onClick={handlePrevMonth} className="hover:bg-blue-50 dark:hover:bg-blue-900/30 rounded-full p-0.5 transition-colors">
                            <ChevronLeft size={16} />
                          </button>
                          <span className="min-w-[120px] text-center text-xs">{currentMonthName}</span>
                          <button onClick={handleNextMonth} className="hover:bg-blue-50 dark:hover:bg-blue-900/30 rounded-full p-0.5 transition-colors">
            ...
            {activeView === 'profile' && (
              <motion.div key="profile" variants={containerVariants} initial="hidden" animate="show" exit="exit" className="max-w-7xl mx-auto grid grid-cols-1 xl:grid-cols-3 gap-6">
                ...
                {/* Profile Overview — reflects saved state */}
                <motion.div variants={itemVariants} className="xl:col-span-1">
                  <Card className="p-6 border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm flex flex-col items-center text-center">
                    ...
                    <div className="relative group mb-4">
                      <div className="w-32 h-32 rounded-full bg-blue-50 dark:bg-blue-900/50 border-4 border-blue-500 shadow-sm flex items-center justify-center overflow-hidden">
                        {profileImage ? (
                          <img src={profileImage} alt="Profile" className="w-full h-full object-cover" />
                        ) : (
                    ...
                    <label className="text-xs font-bold text-blue-600 hover:text-blue-700 cursor-pointer mb-2">
                      Change Picture
                      <input type="file" accept="image/*" className="hidden" onChange={handleImageUpload} />
                    </label>

                    <h3 className="font-bold text-xl text-slate-800 dark:text-slate-100">{user?.name || 'Doctor'}</h3>
                    {/* Specialization reflects what was last saved */}
                ...
                <motion.div variants={itemVariants} className="xl:col-span-2">
                  <Card className="p-6 border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm flex flex-col gap-5 h-full min-h-[400px]">
                    <div>
                      <h3 className="text-lg font-semibold text-slate-800 dark:text-slate-100 mb-1">Edit Profile</h3>
                      <p className="text-sm text-slate-500 dark:text-slate-400">Changes are reflected immediately across the dashboard once saved.</p>
                    </div>

                    {/* Specialization */}
                    <div>
                      <label className="block text-xs font-semibold text-slate-500 dark:text-slate-400 mb-1.5 uppercase tracking-wide">Specialization</label>
                      <input
                        type="text"
                        value={specialization}
                        onChange={e => setSpecialization(e.target.value)}
                        className="w-full bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700/50 text-slate-800 dark:text-slate-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="block text-xs font-semibold text-slate-500 dark:text-slate-400 mb-1.5 uppercase tracking-wide">Experience</label>
                        <input
                          type="text"
                          value={experience}
                    ...
                    <div className="flex flex-col flex-1">
                      <label className="block text-xs font-semibold text-slate-500 dark:text-slate-400 mb-1.5 uppercase tracking-wide">Professional Biography</label>
                      <textarea
                        value={bio}
                    ...
                    <div className="flex justify-end">
                      <Button isLoading={isSavingBio} onClick={handleSaveBio} className="bg-blue-600 hover:bg-blue-700">
                        Save Profile
                      </Button>
                    </div>
                  </Card>
